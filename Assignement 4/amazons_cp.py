...
def amazons_cp(size: int, placed_amazons: list[(int, int)]) -> (bool, list[list[int]]):
    ...
    x = VarArray(size=size, dom=range(-1, size)) # -1 means no amazon, otherwise the index of the row where the amazon is placed in the column i

    # Create the constraints here
    satisfy(

        # Constraints 1 : Placed amazons must be respected
        [x[j] == i for (i, j) in placed_amazons],

        # Constraints 2 : Must be 'size' amazons
        [x[i] != -1 for i in range(size)],

        # Constraints 3 : Only one amazon per row
        [AllDifferent(x)],

        # Constraints 4 : Only one amazon per column
        # No need to write it by construction of the model !

        # Constraints 5 : Only one amazon per diagonal (resp. anti-diagonal)
        [abs(x[i] - x[j]) != abs(i - j) for i in range(size) for j in range(size) if i != j],

        # Constraints 6 : No amazon on 3x2 moves
        [abs(x[i] - x[j]) != 3  for i in range(size) for j in range(size) if abs(i - j) == 2],
        [abs(x[i] - x[j]) != 2  for i in range(size) for j in range(size) if abs(i - j) == 3],

        # Constraints 7 : No amazon on 4x1 moves
        [abs(x[i] - x[j]) != 4  for i in range(size) for j in range(size) if abs(i - j) == 1],
        [abs(x[i] - x[j]) != 1  for i in range(size) for j in range(size) if abs(i - j) == 4]
    )
    ...
    if solve(solver=CHOCO) is SAT:
        ...
        for i in range(size): output[value(x[i])][i] = 1
    else:
        status = False
